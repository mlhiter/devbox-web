...
const (
	...
	sdkServerDialTimeout      = 250 * time.Millisecond
	sdkServerWaitTimeout      = 3 * time.Second
	sdkServerRetryInterval    = 100 * time.Millisecond
	devboxSecretWaitTimeout   = 3 * time.Second
	devboxSecretRetryInterval = 100 * time.Millisecond
...
type createDevboxKubeAccessRequest struct {
...
type createDevboxRequest struct {
	...
	KubeAccess            *createDevboxKubeAccessRequest `json:"kubeAccess,omitempty"`
...
func (s *apiServer) handleCreateDevbox(w http.ResponseWriter, r *http.Request) {
	...
	kubeAccess, err := parseCreateDevboxKubeAccess(req.KubeAccess)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func parseCreateDevboxKubeAccess(raw *createDevboxKubeAccessRequest) (*devboxv1alpha2.KubeAccessSpec, error) {
	if raw == nil {
		return nil, nil
	}

	roleTemplate := devboxv1alpha2.KubeAccessRoleTemplate(strings.TrimSpace(raw.RoleTemplate))
	if roleTemplate != "" {
		switch roleTemplate {
		...
		default:
			return nil, fmt.Errorf("invalid kubeAccess.roleTemplate: must be one of view, edit, admin")
		}
	}

	if !raw.Enabled && roleTemplate == "" {
		return nil, nil
	}

	return &devboxv1alpha2.KubeAccessSpec{
		Enabled:      raw.Enabled,
...
func (s *apiServer) handleGetDevboxInfo(w http.ResponseWriter, r *http.Request) {
	...
	privateKeyBase64, devboxJWTSecret, credentialsPending, err := s.loadDevboxCredentials(r.Context(), namespace, name)
	if err != nil {
		s.logError("get devbox credentials failed", err, "namespace", namespace, "name", name)
		writeError(w, http.StatusInternalServerError, fmt.Sprintf("get devbox credentials failed: %v", err))
		return
	}
	if credentialsPending {
		s.logDebug("devbox credentials not ready yet", "namespace", namespace, "name", name)
	}
...
func (s *apiServer) handleExecDevbox(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.waitForSDKServerReachable(ctx, sdkServerBaseURL); err != nil {
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			s.logWarnError("wait for sdk server before exec timed out", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container, "http_status", http.StatusGatewayTimeout)
			writeError(w, http.StatusGatewayTimeout, "exec command timeout")
			return
		}
		s.logError("wait for sdk server before exec failed", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container)
...
func (s *apiServer) handleUploadFile(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.waitForSDKServerReachable(ctx, sdkServerBaseURL); err != nil {
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			s.logWarnError("wait for sdk server before upload timed out", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container, "path", filePath, "http_status", http.StatusGatewayTimeout)
			writeError(w, http.StatusGatewayTimeout, "upload file timeout")
			return
		}
		s.logError("wait for sdk server before upload failed", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container, "path", filePath)
...
func (s *apiServer) handleDownloadFile(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.waitForSDKServerReachable(ctx, sdkServerBaseURL); err != nil {
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			s.logWarnError("wait for sdk server before download timed out", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container, "path", filePath, "http_status", http.StatusGatewayTimeout)
			writeError(w, http.StatusGatewayTimeout, "download file timeout")
			return
		}
		s.logError("wait for sdk server before download failed", err, "namespace", namespace, "name", name, "pod", pod.Name, "container", container, "path", filePath)
...
func (s *apiServer) loadDevboxPrivateKeyBase64(ctx context.Context, namespace string, devboxName string) (string, error) {
	secret, err := s.waitForDevboxSecret(ctx, namespace, devboxName)
	if err != nil {
		return "", err
	}

	return devboxPrivateKeyBase64FromSecret(secret, s.cfg.SSH, namespace, devboxName)
}
...
func (s *apiServer) loadDevboxJWTSecretByCacheKey(ctx context.Context, namespace string, devboxName string, cacheKey string) (string, error) {
	...
	key, err := devboxJWTSecretFromSecret(secret, namespace, devboxName)
	if err != nil {
		return "", err
	}
...
func (s *apiServer) loadDevboxCredentials(
	ctx context.Context,
	namespace string,
	devboxName string,
) (privateKeyBase64 string, devboxJWTSecret string, credentialsPending bool, err error) {
	secret, err := s.waitForDevboxSecret(ctx, namespace, devboxName)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return "", "", true, nil
	...
	privateKeyBase64, err = devboxPrivateKeyBase64FromSecret(secret, s.cfg.SSH, namespace, devboxName)
	if err != nil {
		return "", "", false, err
	}
	devboxJWTSecret, err = devboxJWTSecretFromSecret(secret, namespace, devboxName)
	if err != nil {
		return "", "", false, err
	}
	s.storeDevboxJWTSecret(
		devboxJWTSecretCacheKey(namespace, devboxName),
		devboxJWTSecret,
		time.Now().Add(jwtSecretCacheTTL),
	)
	return privateKeyBase64, devboxJWTSecret, false, nil
}

func (s *apiServer) waitForDevboxSecret(ctx context.Context, namespace string, devboxName string) (*corev1.Secret, error) {
	if ctx == nil {
		ctx = context.Background()
	}

	deadline := time.Now().Add(s.secretWaitTimeout())
	var lastErr error
	...
	for {
		...
		err := s.ctrlClient.Get(ctx, ctrlclient.ObjectKey{Namespace: namespace, Name: devboxName}, secret)
		if err == nil {
			return secret, nil
		}
		if !apierrors.IsNotFound(err) {
		...
		if time.Now().After(deadline) {
			return nil, fmt.Errorf(
				"secret %s/%s not found after waiting %s: %w",
				namespace,
				devboxName,
				s.secretWaitTimeout(),
				lastErr,
		...
		timer := time.NewTimer(s.secretRetryInterval())
		select {
		case <-ctx.Done():
			if !timer.Stop() {
				<-timer.C
...
func (s *apiServer) secretWaitTimeout() time.Duration {
	if s == nil || s.devboxSecretWaitTimeout <= 0 {
		return devboxSecretWaitTimeout
	}
	return s.devboxSecretWaitTimeout
}

func (s *apiServer) secretRetryInterval() time.Duration {
	if s == nil || s.devboxSecretRetryInterval <= 0 {
		return devboxSecretRetryInterval
	}
	return s.devboxSecretRetryInterval
}

func devboxPrivateKeyBase64FromSecret(
	secret *corev1.Secret,
	cfg SSHConnectionConfig,
	namespace string,
	devboxName string,
) (string, error) {
	keyName := strings.TrimSpace(cfg.PrivateKeySecretKey)
...
func devboxJWTSecretFromSecret(secret *corev1.Secret, namespace string, devboxName string) (string, error) {
...
func (s *apiServer) dialSDKServer(ctx context.Context, network, address string) (net.Conn, error) {
	if s != nil && s.sdkServerDialContext != nil {
		return s.sdkServerDialContext(ctx, network, address)
	}
...
func (s *apiServer) waitForSDKServerReachable(ctx context.Context, sdkServerBaseURL string) error {
	if ctx == nil {
		ctx = context.Background()
	}

	sdkServerURL, err := neturl.Parse(sdkServerBaseURL)
	if err != nil {
		return fmt.Errorf("parse sdk server baseURL: %w", err)
	}

	address := strings.TrimSpace(sdkServerURL.Host)
	if address == "" {
		return fmt.Errorf("sdk server baseURL %q missing host", sdkServerBaseURL)
	}

	deadline := time.Now().Add(sdkServerWaitTimeout)
	var lastErr error
	...
	for {
		...
		probeCtx, cancel := context.WithTimeout(ctx, sdkServerDialTimeout)
		conn, err := s.dialSDKServer(probeCtx, "tcp", address)
		cancel()
		if err == nil {
			_ = conn.Close()
			return nil
		}
		if !isRetriableSDKServerReachabilityError(ctx, err) {
			return err
		}

		lastErr = err
		if time.Now().After(deadline) {
			return fmt.Errorf("sdk server %s is not reachable yet: %w", address, lastErr)
		}

		timer := time.NewTimer(sdkServerRetryInterval)
		select {
		case <-ctx.Done():
			if !timer.Stop() {
				<-timer.C
...
func isRetriableSDKServerReachabilityError(ctx context.Context, err error) bool {
	if err == nil {
		return false
	}

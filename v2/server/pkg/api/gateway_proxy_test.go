...
func TestHandleGatewayProxyForwardsToRunningPodWithoutDNSLookup(t *testing.T) {
...
func TestHandleGatewayProxyReturnsBadGatewayWhenRunningPodHasNoIP(t *testing.T) {
	devbox := &devboxv1alpha2.Devbox{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "demo-devbox",
			Namespace: "ns-test",
	...
	if resp.Code != http.StatusBadGateway {
		t.Fatalf("expected status %d, got %d, body=%s", http.StatusBadGateway, resp.Code, resp.Body.String())
	}
}

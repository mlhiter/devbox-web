...
- `/{gateway.pathPrefix}/{uniqueID}[/*rest]` (reverse proxy to the target devbox pod on port `1317`)
...
- Recommended ingress model:
  ...
  - ingress 2: expose `/codex/*` to the gateway listener on `server.gatewayListenAddress`, then let `v2/server` reverse proxy to the current DevBox pod on port `1317`
...
- Gateway proxy requests resolve the current running DevBox pod and forward to `http://<podIP>:1317`, while preserving the logical upstream host for headers and redirect rewriting.
